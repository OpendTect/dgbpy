...
netgrp.add_argument( '--dlserver',
            dest='dlserver', action='store',
            type=str, default='',
            help='Deep Learning service manager connection details')
...
with dgbservmgr.ServiceMgr(args['dlserver'], args['ppid']) as this_service:
...
# Keep all lengthy operations below
  ...
  def selParsGrp( platformnm ):
    ...
    curdoc().add_root(mainpanel)
    this_service.sendObject('platform_change', platformnm)
  ...
  def doRun( cb = None ):
    ...
    this_service.sendEvent('training_started')
  ...
  def trainMonitorCB( proc ):
    ...
    try:
      ...
      if not odcommon.batchIsFinished( odcommon.get_log_file() ):
        ...
        this_service.sendEvent('training_finished')
      return False
    return True
